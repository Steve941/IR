...
*new*: Den Bund-Future habe ich bei onvista in 5-Jahresstücken geladen und zusammengebaut. Dezimaltrennzeichen umgestellt im .csv
```{r, cache=T}
...
```{r}
ret <- stocks[2:nrow(stocks),2:ncol(stocks)]/stocks[1:(nrow(stocks)-1),2:ncol(stocks)] - 1
...
```{r}
datesEval <- dates[dates >= "2015-06-15" & dates <= "2016-08-19"]
length(datesEval)
```
...
## Fixation of Portfolio at start

We evaluate an optimal portfolio at the starting date of our test season and stick to it.
...
roughly 30 seconds per strategy and weight
```{r}
...
```{r}
E <- list()
tt <- numeric(nrow(grid)*length(sentixDataNamesReg))

for(weightInd in 1:2){
    w <- as.numeric(grid[weightInd,])
    weightName <- paste(w, collapse = "-") # needed later to store result
    
    for(strategy in sentixDataNamesReg[1:3]){
        ...
        erg <- matrix(NA, nrow = length(datesEval)+1, ncol = numAsset) # +1 to lookup every weight
        rownames(erg) <- c("1000-01-01", paste(datesEval))
        erg[1, ] <- rep(1/numAsset, numAsset)
        
        for(d in datesEval){
            dInd <- which(datesEval==d)
