...
rm(anC, anCOpt, anCRisky, anRet, anRetRisky, CRisky, xMarket, anMuRisky, muRisky)
```
...
registerDoSNOW(cl)
xDispConstTest <- foreach(t = datesTestNames, .export = c(datesTestNames), .packages = c("Rdonlp2")) %do%{
    L <- list()
...
for(t in datesTestNames){
    ...
    xDispVarTest[[t]] <- foreach(i = names(sDisp)[1], .export = c(datesTestNames), .packages = c("Rdonlp2")) %dopar%{
        L <- list()
